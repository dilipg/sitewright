...
**Milestones 1–3 are complete — the walking skeleton works end to end** (prompts P0–P2, 1.1–1.5, 2.1–2.4, 3.1–3.4): brief → generated hero (orchestrator, Kitaru-checkpointed, gated retry) → canvas edit → export, proven by the M3 soak ([docs/reports/m3-soak.md](docs/reports/m3-soak.md), 10/10 green at 0.24× the token budget). `compiler/` holds the deterministic spine (token deriver, manifest service, gates + CLI, exporter, manifest CLI, bridge shim + preview server); `editor/` is a working single-frame editor (selection, style channel, persisted undo/redo); `orchestrator/` (Python/uv/Kitaru) holds the model-call/prompt/runlog machinery and the single-section pipeline. The invariant suite ([editor/e2e/invariant.spec.ts](editor/e2e/invariant.spec.ts)) proves preview=export pixel fidelity in `npm run check`/CI; the orchestrator needs `orchestrator/.env` with `ANTHROPIC_API_KEY` for live generation. The build order and exact per-step prompts live in [docs/build-prompts-v1.md](docs/build-prompts-v1.md); work through them in sequence (next: milestone 4 — regeneration + ID survival, the stop-the-line risk). Do not invent structure; follow the docs.
