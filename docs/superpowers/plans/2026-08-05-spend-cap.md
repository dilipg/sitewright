...
Add to `server/src/auth-routes.test.ts` (matching the file's existing style for an authenticated request — reuse its login helper rather than inventing a new one):

```ts
  it("reports the current 24h spend alongside the cap", async () => {
    // ... log in as an existing user through the same helper the file already uses,
    // record a usage event of 3.25 for that user at Date.now(), then GET /api/me
    expect(body.spendCapUsd).toBe(10);
    expect(body.spentUsd24h).toBe(3.25);
  });
...
export function ingestUsageLog(
  ...
  for (const line of text.split("\n")) {
    ...
    const at = Number.isFinite(parsedAt) ? parsedAt : now;
