...
Expected: `server listening on http://localhost:4000`. Then `curl -i -X POST localhost:4000/api/login -H 'Content-Type: application/json' -d '{"email":"a@b.c","password":"x"}'` returns **401** (no user exists yet), and `curl -i localhost:4000/api/nope` returns **404**. Stop the server.

> The explicit `Content-Type` header is required: login rejects any other media
> type with a 400, because an HTML form can only send one of three content
> types and none of them is `application/json` — that is what closes the
> login-CSRF vector (`SameSite=Lax` guards requests that *send* the session
> cookie; login *sets* one). `curl -d` alone defaults to
> `application/x-www-form-urlencoded` and would return 400, not 401.
