...
> **CORRECTION, 2026-08-13 (fix round B, D-3).** This step's projects root, and Step 5's "`--projects-root` (default `./generated`)", both read as a **free choice**. They are not one any more, and the instruction as written **cannot be followed today** — appended rather than rewritten, per the convention the 2026-08-10 F13 retraction set.
>
> Since slice 5 (decisions.md, 2026-08-09) `JobWorker`'s constructor **refuses to build** — so `scripts/serve.ts` exits non-zero — unless `resolve(projectsRoot)` is the orchestrator's own `generated/` (`assertProjectsRootMatchesOrchestrator`). A scratch root outside the repo therefore fails at boot, and that non-zero exit is the guard working. The reason is that `section_pipeline.py`'s `GENERATED_DIR = REPO_ROOT / "generated"` is an absolute module constant with no argument and no env override: any other value meant a ~400s, ~$1 generation finishing `succeeded` into a directory the preview pool would never look at.
>
> So `--projects-root` is **effectively single-valued**. Threading `--out-dir` through the orchestrator is the recorded follow-up (`pending.md` H6); until then, pass the repo's own `generated/`. The rest of this step (two users, adoption counts, the 404 comparison) is unaffected — only the *location* is.
