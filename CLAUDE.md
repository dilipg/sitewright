...
**Milestones 1–4 and 5.1–5.3 are complete** (prompts P0–P2, 1.1–1.5, 2.1–2.4, 3.1–3.4, 4.1–4.3, 5.1–5.3). The walking skeleton works end to end (brief → generated hero → canvas edit → export; [m3-soak.md](docs/reports/m3-soak.md): 10/10 at 0.24× budget), section regeneration works via Kitaru replay-with-overrides with gate 7 + the orphan dialog + revert, and **the stop-the-line risk is retired**: the ID-survival stress suite ([m4-id-survival.md](docs/reports/m4-id-survival.md)) measured 100% auto-reattach (71/71) with zero silent drops across 20 varied regen instructions. Milestone 5 built out the full pipeline: intake + planner + plan approval (5.1), the Design System Agent (5.2), and the Shell Agent + parallel page fan-out (5.3) — one worker subprocess per route, sections generated sequentially within a page as independent, checkpoint-resumable flow executions, a deterministic per-page assembly step, and gate 6's dynamic ownership-boundary check verified clean via a live 4-page parallel run; a controlled kill -9-mid-section crash-resume test confirmed no duplicate model spend and no manifest garbage on resume. That live run also surfaced and fixed a cluster of real gaps: gate 4 now recognizes contract-5.2 list-item node IDs built via a template literal inside `.map()` (including through a local `const` indirection, not just inlined), `skipMissingCheck` now exempts only section-root IDs instead of blanket-skipping child IDs pre-assembly, and the orchestrator now defends against Claude's tool-use not hard-enforcing its own schema under retry pressure (a section's own manifest-root proposal going missing; `files`/`manifestProposals` arriving malformed) — see decisions.md's 2026-07-28 rows for the full account, including the operational lesson that a Kitaru `run_id` must not be reused across a checkpoint-function code edit. `compiler/` holds the deterministic spine (token deriver, manifest service incl. replaceSection + a cross-process file lock + targeted rollback-commit, gates 1–7 + CLIs, exporter, bridge shim + preview server with overrides/regen APIs); `editor/` covers selection, style channel, persisted undo/redo, and the regen UX; `orchestrator/` (Python/uv/Kitaru, needs `orchestrator/.env` with `ANTHROPIC_API_KEY`; `GEMINI_API_KEY` + `ORCH_MODEL_PROVIDER=gemini` is an opt-in escape hatch, default stays Anthropic) holds prompts/runlog/generation/regeneration/soak/stress/shell/fan-out. The build order and exact per-step prompts live in [docs/build-prompts-v1.md](docs/build-prompts-v1.md); work through them in sequence (next: milestone 5.4 — six archetype templates + remaining edit channels + infinite canvas). Do not invent structure; follow the docs.
