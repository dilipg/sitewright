 ...
 * Output: a JSON result { ok, issues } on stdout. Exit 0 valid / 1 invalid.
...
import { commit, createManifest, propose } from "../src/manifest.ts";
...
if (
  (command !== "propose" && command !== "commit") ||
  projectDir === undefined ||
  proposalsFlag === -1 ||
  ownerFlag === -1
) {
  console.error("Usage: manifest <propose|commit> <projectDir> --proposals <file.json> --owner <owner>");
  process.exit(2);
}
