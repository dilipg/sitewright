 ...
 *
 * WHY THIS EXISTS, and it is not hypothetical: task 1 of the BYOK/Docker plan (in `server/`, but the same hazard lives here:
 * `compiler/scripts/preview.ts` and `export.ts` are both run directly by Node)
