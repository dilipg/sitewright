  ...
  try {
    ...
    for (const line of usableLines) {
      ...
      try {
        ...
        // Last line of defence for the INSERT specifically: guards write-lock
        // contention past db.ts's busy_timeout and any future constraint this
        // schema grows, not only a stale userId (that path is already closed
        // above). Every KNOWN failure mode is guarded before this point; this
    ...
    // The outer backstop. `findUserById` and `findProjectById` above are
    // unguarded `db.prepare(...).get()` calls — unlike the file read and the
    // per-row insert, nothing wraps them individually. A SQLite exception at
    // either (write-lock contention past db.ts's documented cross-process
    // busy_timeout, a closed or corrupted handle) would otherwise propagate
